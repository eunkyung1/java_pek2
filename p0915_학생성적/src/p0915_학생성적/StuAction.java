...
public class StuAction {
	...
	int stuInput(int stuCount) {
		while(true) {
			...
			System.out.printf("%d 번째 학생성적이 저장되었습니다.",stuCount);
			System.out.println();
			stuCount++;
		}//while
		return stuCount;
	...
	void stuOutput() {
		...
		void stuUpdate() {
			while(true) {
				...
				for(int i=0;i<list.size();i++) {
					StuScore s = (StuScore) list.get(i);
					if(name.equals(s.getName())) { //찾은 방 번호
						count = i;//찾은 방 번호
						...
						System.out.println("--------------------------");
						System.out.println("원하는번호를 선택하세요.");
						int choice = scan.nextInt();
						...
						case 1:
							...
							s.setTotal(s.getKor()+s.getEng()+s.getMath());//점수를 읽은 후 바로 합계저장
				...
				if(count ==-1) {
					System.out.printf("※※ 찾고자 하는 %s 학생이 없습니다.※※ 다시 검색하세요. \n",name);
					System.out.println();
				}//if
			}//while
		}//stuUpdate
